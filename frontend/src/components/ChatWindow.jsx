...
const CHAT_MESSAGES_QUERY = gql`
  query ChatMessages($chat_id: uuid!) {
      ...
      <div className={`flex-1 flex items-center justify-center ${isDark ? 'bg-gray-800' : 'bg-gray-50'
        ...
        >
          <div className={`mx-auto h-16 w-16 rounded-full flex items-center justify-center mb-4 ${isDark ? 'bg-gray-700' : 'bg-gray-200'
            ...
            <svg className={`h-8 w-8 ${isDark ? 'text-gray-400' : 'text-gray-500'
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {data?.messages?.length === 0 ? (
            ...
            >
              <div className={`mx-auto h-12 w-12 rounded-full flex items-center justify-center mb-4 ${isDark ? 'bg-gray-700' : 'bg-gray-200'
                ...
                <svg className={`h-6 w-6 ${isDark ? 'text-gray-400' : 'text-gray-500'
                  }`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <AnimatePresence>
          {isTyping && (
            ...
            >
              <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-2xl rounded-bl-md ${isDark
                  ? 'bg-gray-700 text-gray-100'
                  : 'bg-white text-gray-900 shadow-sm border border-gray-200'
                }`}>
                <div className="flex space-x-1">
                  <div className={`w-2 h-2 rounded-full animate-bounce ${isDark ? 'bg-gray-400' : 'bg-gray-500'
